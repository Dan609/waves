...
The “mother” Morlet wavelet is: \(\psi(t)=\pi^{-frac{1}{4}}e^{\textit{i\omega t}}e^(-frac{\textit{t}^2}{2})\). The Morlet wavelet transform of a time series ($ \textit{x}_t $) is defined as the convolution of the series with a set of “wavelet daughters” generated by the mother wavelet by translation in time by $\tau$ and scaling by \textit{s}: $$Wave(\tau, \textit{s})=\sum_{textit{t}}\textit{x}_{\textit{t}}\frac{1}{\sqrt{s}\psi^{\ast}\left(frac{textit{t}-\tau}{textit{s}}\right)}$$
