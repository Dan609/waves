...
In this formula~\ref{eq:morlet} “angular frequency” $\omega$ is set to 6 since it makes the Morlet wavelet approximately analytic.
...
In formula~\ref{eq:wave} (2) $\tau$ is the localizing time parameter defines position of the particular daughter wavelet in the time domain, and the set of scales $s$ determines the wavelet coverage of the series in the frequency domain.
...
Displacements of periodic phenomena relative to the localizing origin τ, shifted across the time domain, are given by the instantaneous or local wavelet phase.
...
\text{Coherence}=\frac{|\text{sWave.xy}|^2}{\text{sPower.x}\cdot\text{xPower.y}}
\end{equation}
