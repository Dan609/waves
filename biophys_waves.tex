        ...
        for (i in 1:l){
                tryCatch({
                        ...
                        peaks <- fpeaks(spec(calcium_ts, f=s.rate),
                                        nmax = 10, threshold = 0.1, # set nmax for peaks quantity
...
ggplot(df.freq, aes(reorder(ROI, freq), freq)) +
        ...
        geom_hline(yintercept=7.071105, linetype="dashed", color = "black", size = 1, show.legend = TRUE) +
...
calcium <- read.csv('fluoPxF0.csv', sep = ',') # Fluo-4 F0 normalazed signal >>>>JUMP TO total
...
calcium <- read.csv('aneppsPxF0.csv', sep = ',') # Di-8-ANEPPS F0 normalazed signal
colnames(calcium)[1] <- "frame" # rename first column
boxplot(calcium[,-1])
#------------------------------------------
...
plot(spy ~ spx, xlab="frequency (Hz)", ylab="spectral density",
     type="l", lwd = 2, main = 'Fluo-4 512x512 px', xlim=c(0,7), ylim = c(0, 10))
