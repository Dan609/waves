...
ggplot(total,
       aes(x = F.int,
        ...
        scale_y_continuous(name="Anepps intensity, a.u.", limits=c(0, 50)) +
...
corr.calc <- function(x){
        x <- cor.test(~ x[,4] + x[,5], x)
        return(x$estimate)}
tail(total)

corr.calc(total)

Data.num <- total[,c(4,5)]
...
# END
#
calcium <- read.csv('fluoF0.csv')
colnames(calcium)[1] <- "frame" # rename first column
# calcium <- read.csv('anepps.csv', sep = ',') # Di-8-ANEPPS raw signal
calcium <- read.csv('aneppsRaw.csv', sep = ',') #Di-8-ANEPPS F0 normalazed
colnames(calcium)[1] <- "frame" # rename first column
boxplot(calcium[,-1])
#------------------------------------------
...
xcal <- calcium$Y555# Enter Y(ROI - 1) num #<<<<<<<<<<<<<<<<<<<CHANGE VAR
