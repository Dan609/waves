...
\ce{Ca^2+} ions play a key role in the formation of the electrical and mechanical responses of the heart.
...
The source of these waves are local high-frequency oscillations \ce{Ca^2+} release (LHFOCR) regions, associated with hypercontraction-related damage sites.
...
\begin{figure}[hbt!]
\centering
\includegraphics[width=0.6\linewidth]{fig0.png}
...
\begin{figure}
    ...
    \caption{Heat maps of 30x10 px ROI manually selected for ...}
    \label{fig:fig6}
\end{figure}
...
calcium <- read.csv2('40cells_Fluo-4.csv')
boxplot(raw, main = 'calcium')
head(calcium)

anepps <- read.csv2('40cells_di-8-anepps.csv')
boxplot(raw, main = 'anepps')
head(anepps)

norm <- read.csv2('094_F0-normalized.csv', sep = ',')
boxplot(raw, main = 'Raw')

raw <- read.csv2('094_raw.csv', sep = ',')
boxplot(norm, main = 'Normalized')


# Remove Time column before reordering
anepps_roi <- anepps[, -1]
boxplot(anepps_roi, main = 'Di-8-ANEPPS')
anepps_roi <- anepps_roi[,order(mean)]


# Remove Time column before reordering
calcium_roi <- calcium[, -1]
head(calcium_roi)
boxplot(calcium_roi, main = 'Fluo-4')

# Reorder a data frame's columns according to arithmetic mean of each column.
df <- calcium_roi
mean <- colMeans(df, na.rm=TRUE)
df <- df[,order(mean)]
boxplot(df, main = 'Fluo-4')

calcium <- read.csv2('40cells.csv')
# raw signal
calcium <- read.csv2('094raw.csv', sep=',')

# F0 normalizes signal
...
# colnames(calcium) <- c("frame",
#                       'ROI_512x512', 'ROI_180x40', 'ROI_40x40',
#                        'ROI_2x2', 'ROI_10x10', 'ROI_2x2_2')
...
spec(z, f=s.rate, wl = 512, wn = "hanning", fftw = FALSE, norm = TRUE,
     scaled = FALSE, PSD = FALSE, PMF = FALSE,
     correction="none", dB = NULL, dBref = NULL,
